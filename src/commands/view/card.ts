...
import { formatTime, noAuthResponse, noBoardSelectedResponse, stripIndentsAndNewlines, toColorInt, truncateList } from '../../util';
...
export default class CardCommand extends SlashCommand {
  ...
  async run(ctx: CommandContext) {
      ...
      fields: [{
        ...
        value: stripIndentsAndNewlines`
          ...
          **${t('common.list')}:** ${truncate(board.lists.find(list => card.idList === list.id).name, 50)}
          ...
          ${card.membersVoted.length ? `**${t('common.votes')}:** ${formatNumber(card.membersVoted.length, userData.locale)}${hasVoted ? ` ${t('card.vote_include')}` : ''}` : ''}
        `
      }]
    };
    ...
    // Labels
    if (card.labels.length) embed.fields.push({
    ...
    // Attachments
    if (card.attachments.length) embed.fields.push({
        name: t('common.attachments'),
        value: truncateList(card.attachments.map(
          (attachment) => `[${truncate(attachment.name, 20)}](${attachment.url})`
        ), t),
        inline: true
      });
    ...
    if (card.stickers.length) {
      ...
      embed.fields.push({
        ...
        value: Object.keys(stickers).map((key) =>
          `${STICKER_EMOJIS[key] ? `<:_:${STICKER_EMOJIS[key]}>` : key}${
            stickers[key] > 1 ? ` ${formatNumber(stickers[key], userData?.locale)}` : ''}`).join(' '),
        inline: true
      });
    }

    // Checklists
    if (card.checklists.length) embed.fields.push({
        name: t('common.checklists'),
        value: truncateList(card.checklists.map(
          (checklist) => {
            ...
            return `${completed ? EMOJIS.check : EMOJIS.uncheck} ${truncate(checklist.name, 50)} (${formatNumber(checkedCount, userData.locale)}/${formatNumber(checklist.checkItems.length, userData.locale)})`;
          }
        ), t),
        inline: true
      });
    ...
    // Members
    if (card.members.length) embed.fields.push({
        name: t('common.members'),
        value: truncateList(card.members.map(
          (member) => userData.trelloID === member.id ? `**${(member.fullName || member.username)}**` : (member.fullName || member.username)
        ), t),
        inline: true
      });
